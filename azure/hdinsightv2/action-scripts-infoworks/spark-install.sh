...
_init(){
	...
	if [ ${HDP_VERSION} == "2.5.6.3-5" ]; then
		_download_file ${public_blob}'sparkconf3-5.tar.gz' /sparkconf.tar.gz
	else
		_download_file ${public_blob}'sparkconf3-6.tar.gz' /sparkconf.tar.gz
	fi
	_download_file ${public_blob}'webapps.tar.gz' /webapps.tar.gz
