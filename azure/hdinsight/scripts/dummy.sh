...
su -c 'pushd /opt/iw-installer && source configure.sh && ./configure_install.sh && ./install.sh -v 3.2.1_beta1-adb-ubuntu && popd || echo "failed" > /tmp/iwstatus' -s /bin/bash $DF_USER
